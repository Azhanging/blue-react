...
import {THistory, THistoryRouteMeta} from '$components/BrRoutes/types';

//路由挂载后执行（和vue项目不一致，所有的meta在Routes render中才能获取到rawRoute的信息）
function routerAfter ( opts: { history: THistory } ) {
	const {history} = opts;
...
//after后钩子
function afterHook ( opts: { meta: THistoryRouteMeta } ) {
	const {meta} = opts;
