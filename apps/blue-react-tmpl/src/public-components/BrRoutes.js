		...
		_routes.push((
			<Route
				...
				render={(routeProps) => {
					...
					if (history.action === 'POP' && history.location.key === undefined) {
					...
					//设置组件缓存
					cache.list[location.key] = component;

					if (cache.list[location.key]) {
						console.log(cache.list[location.key]);
						return cache.list[location.key];
					}
